...
import { Search, Grid3x3, List, Flag, ChevronLeft, ChevronRight } from "lucide-react"
...
export default function GolfPage() {
  ...
  const { products: lifestyle, loading: l1, error: e1 } = useProducts({ categoryId: 6, perPage: 30 })
  const { products: outdoor, loading: l2, error: e2 } = useProducts({ categoryId: 5, perPage: 30 })
  const { addToCart } = useCart()
  const { addToWishlist, removeFromWishlist, isInWishlist } = useWishlist()

  const loading = l1 || l2
  const error = e1 || e2
  const allProducts = [...lifestyle, ...outdoor].filter((p, i, arr) => arr.findIndex(x => x.id === p.id) === i)
  ...
  return (
    <PageLayout activePage="sports" seoTitle="Golf Gear | Under Armour®" seoDescription="Shop Under Armour golf gear. Performance polos, pants, and footwear for the course.">
      <main className="flex-1 bg-gray-100">
        <section className="relative bg-black text-white py-12 lg:py-16 overflow-hidden">
          <div className="container mx-auto px-4">
            <div className="max-w-3xl">
              ...
              <p className="text-lg text-gray-300 font-bold">Performance apparel and footwear built for the course.</p>
            </div>
          </div>
        </section>
          ...
          <div className="container mx-auto px-4 py-4">
            <div className="flex flex-col md:flex-row gap-4 items-center">
              <div className="relative flex-1 w-full">
                ...
                <Input type="text" placeholder="Search golf gear..." value={searchQuery} onChange={e => { setSearchQuery(e.target.value); setCurrentPage(1) }} className="pl-10 border-2 border-gray-300 focus:border-red-600 font-bold" />
              </div>
        ...
        <section className="container mx-auto px-4 py-6">
          <p className="font-bold text-gray-700">{loading ? 'Loading...' : `Showing ${paginated.length} of ${filtered.length} golf products`}</p>
