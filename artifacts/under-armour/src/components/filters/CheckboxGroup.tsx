...
export default function CheckboxGroup({ label, options, selected, onChange, columns = 1 }: CheckboxGroupProps) {
  return (
    <div>
        ...
        {options.map((opt) => {
          ...
          return (
            <label key={opt} className="ua-checkbox-label">
              ...
              <span className={checked ? "ua-checkbox-text-checked" : "ua-checkbox-text"}>{opt}</span>
            </label>
          )
        })}
      </div>
