...
import { shoesProducts, shoesCategories, shoesGenders, shoesSizes, shoesColors, shoesTechnologies } from "@/data/products/shoes"
import {
...
export default function ShoesPage() {
  ...
  const filteredProducts = useMemo(() => {
    let filtered = shoesProducts.filter(product => {
    ...
    return filtered
  }, [searchQuery, selectedCategories, selectedGenders, selectedSizes, selectedColors, selectedTechnologies, priceRange, sortBy])
  ...
  const clearFilters = () => {
    ...
    setSelectedColors([]); setSelectedTechnologies([]); setPriceRange([0, 250])
  ...
  return (
    <PageLayout activePage="shoes" seoTitle="Athletic Shoes &amp; Footwear | Under Armour® HOVR™" seoDescription="Shop UA's full lineup of running shoes, training shoes, basketball sneakers, and golf shoes.">
      <main className="flex-1 bg-gray-100">
          ...
          <div className="container mx-auto px-4 py-4">
            <div className="flex flex-col md:flex-row gap-4 items-center">
              <div className="relative flex-1 w-full">
                ...
                <Input type="text" placeholder="Search shoes..." value={searchQuery}
            ...
            <div className="container mx-auto px-4 py-6">
              <div className="grid md:grid-cols-5 gap-6">
                  ...
                  <div className="space-y-2">
                    {shoesCategories.map(cat => (
                      <label key={cat} className="flex items-center cursor-pointer">
                        <input type="checkbox" checked={selectedCategories.includes(cat)} onChange={() => toggle(setSelectedCategories, cat)} className="mr-2 w-4 h-4 accent-red-600" />
                  ...
                  <div className="space-y-2">
                    {shoesGenders.map(g => (
                      <label key={g} className="flex items-center cursor-pointer">
                        <input type="checkbox" checked={selectedGenders.includes(g)} onChange={() => toggle(setSelectedGenders, g)} className="mr-2 w-4 h-4 accent-red-600" />
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {shoesTechnologies.map(tech => (
                      <label key={tech} className="flex items-center cursor-pointer">
                        <input type="checkbox" checked={selectedTechnologies.includes(tech)} onChange={() => toggle(setSelectedTechnologies, tech)} className="mr-2 w-4 h-4 accent-red-600" />
                ...
                <div>
                  ...
                  <input type="range" min="0" max="250" value={priceRange[1]} onChange={(e) => setPriceRange([0, parseInt(e.target.value)])} className="w-full accent-red-600" />
                  <div className="mt-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {shoesColors.map(color => (
                        <button key={color} onClick={() => toggle(setSelectedColors, color)}
                          className={`px-2 py-1 border-2 font-bold text-xs rounded-full transition-colors ${selectedColors.includes(color) ? 'bg-red-600 text-white border-red-600' : 'bg-white border-gray-300 hover:border-red-600'}`}>
                          {color}
                        </button>
                      ))}
                    </div>
        ...
        <section className="container mx-auto px-4 py-6">
          <p className="font-bold text-gray-700">Showing {paginatedProducts.length} of {filteredProducts.length} shoes</p>
        </section>
                    ...
                    <Card className="sketchy-card bg-white border-4 border-black transform hover:scale-105 transition-all duration-300 group h-full flex flex-col cursor-pointer rounded-2xl">
                      <CardContent className="p-6 flex flex-col flex-1">
                        <div className="relative sketchy-frame bg-gray-900 p-4 mb-4 h-64 flex items-center justify-center overflow-hidden rounded-xl">
                          ...
                          {!product.inStock && <div className="absolute inset-0 bg-black bg-opacity-70 flex items-center justify-center z-10 rounded-xl"><span className="text-white font-black text-xl uppercase">Out of Stock</span></div>}
                        ...
                        <div className="space-y-3 flex-1 flex flex-col">
                          <div className="flex gap-2">
                            <div className="sketchy-border bg-red-600 inline-block px-3 py-1 rounded-lg">
                              <span className="text-white font-bold text-xs uppercase">{product.category}</span>
                            </div>
                            {product.technology && (
                              <div className="sketchy-border bg-black inline-block px-3 py-1 rounded-lg">
                      ...
                      <CardContent className="p-6">
                        <div className="flex gap-6">
                          <div className="relative sketchy-frame bg-gray-900 p-4 w-48 flex-shrink-0 rounded-xl">
                            {product.isNew && <Badge className="absolute top-2 left-2 bg-red-600 font-black uppercase z-10 rounded-full">New</Badge>}
                            {product.isSale && <Badge className="absolute top-2 left-2 bg-green-600 font-black uppercase z-10 rounded-full">Sale</Badge>}
                            {!product.inStock && <div className="absolute inset-0 bg-black bg-opacity-70 flex items-center justify-center z-10 rounded-xl"><span className="text-white font-black text-sm uppercase">Out of Stock</span></div>}
                          ...
                          <div className="flex-1 flex flex-col justify-between">
                            <div>
                              ...
                              <p className="text-sm font-bold text-gray-600 mb-2">{product.gender}'s Shoe</p>
                              <StarRating rating={product.rating} reviews={product.reviews} size="md" />
                            ...
                            <div className="flex items-center justify-between mt-4">
                              ...
                              <div className="flex gap-2">
                                <Button variant="outline" size="icon" className="border-2 border-black hover:bg-red-600 hover:text-white rounded-xl"><Heart className="h-5 w-5" /></Button>
