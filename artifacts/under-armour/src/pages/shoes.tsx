...
export default function ShoesPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return items
  }, [searchQuery, selectedCategories, selectedGenders, selectedSizes, selectedColors, selectedTechs, priceRange, sortBy])

  const totalPages = Math.ceil(filtered.length / ITEMS_PER_PAGE)
  const paginated = filtered.slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE)
  const activeCount = selectedCategories.length + selectedGenders.length + selectedSizes.length + selectedColors.length + selectedTechs.length

  const clearFilters = () => {
    setSelectedCategories([]); setSelectedGenders([]); setSelectedSizes([])
    setSelectedColors([]); setSelectedTechs([]); setPriceRange([0, 250])
                  ...
                  <Link key={p.id} href={`/product/${p.id}`}>
                    <div className="bg-white border-2 border-gray-200 hover:border-red-500 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 group h-full flex flex-col cursor-pointer">
                      <div className="relative bg-gray-900 h-60 overflow-hidden">
                        ...
                        {p.technology && <Badge className="absolute bottom-3 left-3 bg-black/80 text-white font-black uppercase z-10 rounded-full text-xs">{p.technology}</Badge>}
                      ...
                      <div className="p-5 flex flex-col flex-1 gap-2">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-bold text-gray-500">{p.gender}</span>
                        </div>
                        ...
                        <div className="flex-1 flex flex-col justify-between py-1">
                          <div>
                            <div className="flex items-center gap-2 mb-1">
                              <span className="text-xs font-black uppercase text-red-600">{p.category}</span>
                              {p.technology && <span className="text-xs font-black bg-black text-white px-2 py-0.5 rounded-full">{p.technology}</span>}
                              <span className="text-xs font-bold text-gray-500">{p.gender}</span>
                            </div>
                            <h3 className="font-black text-xl text-black uppercase mb-2">{p.name}</h3>
                            <StarRating rating={p.rating} reviews={p.reviews} size="sm" />
                            <div className="flex flex-wrap gap-1 mt-3">
                              {p.size.slice(0, 8).map((s) => <span key={s} className="px-2 py-0.5 border border-gray-200 rounded text-xs font-bold text-gray-600">{s}</span>)}
                            </div>
                          </div>
